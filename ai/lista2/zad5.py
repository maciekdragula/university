...
def aStar(state):
    ...
    maxdist = max({dist[(x, y)] for (x, y) in starts})
    vis = {starts : 0}
    Q = PriorityQueue()
    Q.put((maxdist, starts, ''))
    while Q.empty() == False:
        ...
        for (a, b) in moves:
            ...
            maxdist = max({dist[(x, y)] for (x, y) in newPositions})
            if newPositions not in vis or len(path) + 1 < vis[newPositions]:
                vis[newPositions] = len(path) + 1
                Q.put((maxdist + len(path) + 1, newPositions, path + moves[(a, b)]))
    return 'IMPOSSIBLE'
